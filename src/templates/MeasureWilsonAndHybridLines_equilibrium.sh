...
mpirun -n $processes \
	$exename \
	$mode $Nx $Ny $Nz $TimeSpacing $ax $ay $az $StartTime $TimeRange $Rmax $RandomSeed $Beta $nefieldinit $nequilibriumtimesteps $MeasureHybridLoops $iterativeTol $Mass $c0_re $c0_im $c1_re $c1_im $c2_re $c2_im $c3_re $c3_im $FileName_WilsonLoops $FileName_HybridLoops $FileName_FreeStepHybridLoops $FileName_FreeHybridLoops $FileName_QNorm $FileName_ANorm $MeasureEnergy $FileName_Energy
